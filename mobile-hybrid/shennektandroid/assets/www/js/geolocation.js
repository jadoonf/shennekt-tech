var getCurrentPosition = function() {
	...
	var success = function(pos) {
		...
		try{ 
			server = 'http://54.213.101.106/shennekt/Location.php';
		//	var imageURI = $("#status").val();
			console.log("Server "+server);
			if (server) {
				$.ajax({
					type: "POST",
					url: "http://54.213.101.106/shennekt/Location.php",
					data: { 'longitude': pos.coords.longitude,'latitude': pos.coords.latitude },
					cache: false,
					success: function()
					{
						console.log("order submitted");// alert("Order Submitted");
					},
					error: function()
					{
						alert("Error");
